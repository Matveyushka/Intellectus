...
const generateProblemDescription = () => shuffle(
  [...Array(Object.keys(deformationTypes).length).keys()],
).map(item => [item]);

const generateWrongOption = () => {
  const randomKeys = shuffle([...Array(Object.keys(deformationTypes).length).keys()]);
